...
{
    public class CRM_Trader : BaseDataAccess<FineOffice.Entity.CRM_Trader>, FineOffice.IDataAccess.CRM_Trader
    {
        public List<Modules.CRM_Trader> GetList(Modules.Helper.ChangeTrackingList<Modules.Helper.EntitySearcher> changeTrackingList)
        {
            ...
            strSql.Append("FROM vw_crm_trader WHERE 1=1 ");
            if (changeTrackingList.Count > 0)
            {
                strSql.Append(" AND ");
                strSql.Append(JointSearcher.BuidSQL(changeTrackingList));
            }
            List<Modules.CRM_Trader> list = new List<Modules.CRM_Trader>();
            try
            {
                DataTable dt = DbHelperSQL.Query(strSql.ToString()).Tables[0];
                list = FineOffice.Common.CollectionHelper.ConvertTo<FineOffice.Modules.CRM_Trader>(dt) as List<FineOffice.Modules.CRM_Trader>;
            }
            catch (Exception ex)
            {
        ...
        public int GetCount(Modules.Helper.ChangeTrackingList<Modules.Helper.EntitySearcher> changeTrackingList)
        {
            ...
            str.Append("FROM crm_trader WHERE 1=1 ");
            int count = 0;
            if (changeTrackingList.Count > 0)
            {
                str.Append(" AND ");
