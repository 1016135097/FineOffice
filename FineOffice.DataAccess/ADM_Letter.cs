...
{
    public class ADM_Letter : BaseDataAccess<Entity.ADM_Letter>, IDataAccess.ADM_Letter
    {
        public List<Modules.ADM_Letter> GetList(Modules.Helper.ChangeTrackingList<Modules.Helper.EntitySearcher> changeTrackingList)
        {
            ...
            strSql.Append("FROM vw_adm_letter WHERE 1=1 ");
            if (changeTrackingList.Count > 0)
            {
                strSql.Append(" AND ");
                strSql.Append(JointSearcher.BuidSQL(changeTrackingList));
            }
            List<Modules.ADM_Letter> list = new List<Modules.ADM_Letter>();
            try
            {
                DataTable dt = DbHelperSQL.Query(strSql.ToString()).Tables[0];
                list = FineOffice.Common.CollectionHelper.ConvertTo<FineOffice.Modules.ADM_Letter>(dt) as List<FineOffice.Modules.ADM_Letter>;
            }
            catch (Exception ex)
            {
        ...
        public int GetCount(Modules.Helper.ChangeTrackingList<Modules.Helper.EntitySearcher> changeTrackingList)
        {
            ...
            str.Append("FROM adm_letter WHERE 1=1 ");
            int count = 0;
            if (changeTrackingList.Count > 0)
            {
                str.Append(" AND ");
