...
{
    public class SYS_User : BaseDataAccess<FineOffice.Entity.SYS_User>, FineOffice.IDataAccess.SYS_User
    {
        public List<Modules.SYS_User> GetList(Modules.Helper.ChangeTrackingList<Modules.Helper.EntitySearcher> changeTrackingList)
        {
            ...
            strSql.Append("FROM vw_sys_user WHERE 1=1 ");
            if (changeTrackingList.Count > 0)
            {
                strSql.Append(" AND ");
                strSql.Append(JointSearcher.BuidSQL(changeTrackingList));
            }
            List<Modules.SYS_User> list = new List<Modules.SYS_User>();
            try
            {
                DataTable dt = DbHelperSQL.Query(strSql.ToString()).Tables[0];
                list = FineOffice.Common.CollectionHelper.ConvertTo<FineOffice.Entity.SYS_User>(dt) as List<FineOffice.Modules.SYS_User>;
            }
            catch (Exception ex)
            {
        ...
        public List<Modules.SYS_User> GetList(Modules.Helper.ChangeTrackingList<Modules.Helper.EntitySearcher> changeTrackingList, int start, int records)
        {
            ...
            try
            {
                ...
                list = FineOffice.Common.CollectionHelper.ConvertTo<FineOffice.Modules.SYS_User>(dt) as List<FineOffice.Modules.SYS_User>;
            }
            catch (Exception ex)
            {
                throw new Exception(ex.Message);
        ...
        public int GetCount(Modules.Helper.ChangeTrackingList<Modules.Helper.EntitySearcher> changeTrackingList)
        {
            ...
            str.Append("FROM vw_sys_user WHERE 1=1 ");
            int count = 0;
            if (changeTrackingList.Count > 0)
            {
                str.Append(" AND ");
