...
public partial class HardDisk_FrmMyHardDisk : PageBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            btnCopyToPublic.OnClientClick = fileGrid.GetNoSelectionAlertReference(string.Format("请选择要复制的{0}！", fileGrid.Title));
    ...
    protected void btnNewFolder_Click(object sender, EventArgs e)
    {
        ...
        folderWin.Title = "新增";
        folderWin.IFrameUrl = "FrmNewFolder.aspx?parentid=" + folderID;
    ...
    protected void btnMoveFolder_Click(object sender, EventArgs e)
    {
        ...
        ViewState["Expanded"] = tvwFolder.GetExpandedNodeIDs();
        moveWin.Title = "移动至";
        moveWin.IFrameUrl = "FrmMoveFolder.aspx?id=" + folderID;
    ...
    private void fileGrid_Bind(ChangeTrackingList<EntitySearcher> changeTrackingList, string sortColumn, string dortDirection)
    {
        ...
        search.Relation = "AND";
        search.Operator = "=";
        search.Content = CookiePersonnel.ID.ToString();
    ...
    protected void btnMoveFile_Click(object sender, EventArgs e)
    {
        ...
        moveWin.Title = "移动文件至";
        moveWin.IFrameUrl = string.Format("../HardDisk/FrmMoveFiles.aspx?files={0}", files);
        moveWin.Hidden = false;
    }
    ...
    /// 批量移动文件到共享
    /// </summary>
    protected void btnCopyToPublic_Click(object sender, EventArgs e)
    {
        int[] keys = fileGrid.SelectedRowIndexArray;
        List<string> ids = new List<string>();
        for (int i = 0; i < keys.Length; i++)
        ...
        string files = CommonSerialize.Serialize(ListToJson(ids));
        moveWin.Title = "复制到共享硬盘";
        moveWin.IFrameUrl = string.Format("../HardDisk/FrmCopyToPublic.aspx?files={0}", files);
        moveWin.Hidden = false;
    }
