...
public partial class Contract_FrmExpireContract : PageBase
{
    ...
    FineOffice.BLL.SYS_Parameter parameterBll = new FineOffice.BLL.SYS_Parameter();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            btnDelelte.OnClientClick = contractGrid.GetNoSelectionAlertInParentReference(string.Format("请选择要删除的{0}!", contractGrid.Title));
            btnModify.OnClientClick = contractGrid.GetNoSelectionAlertInParentReference(string.Format("请选择要编辑的{0}!", contractGrid.Title));
            btnDelelte.ConfirmText = string.Format("你确认要删除选中的{0}吗!", contractGrid.Title);
            btnDelelte.ConfirmTarget = Target.Parent;
    ...
    protected void LoadData()
    {
        ...
        txtExpire.Text = parameterBll.GetModel(d => d.Sign == "ExpireContract").ParameterValue;
    }
    ...
    {
        FineOffice.Modules.SYS_Parameter model = parameterBll.GetModel(d => d.Sign == "ExpireContract");
        model.ParameterValue = txtExpire.Text;
        parameterBll.Update(model);
        btnFind_Click(null, null);
    }
